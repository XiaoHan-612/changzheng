...
40. **全局快捷键必须让开输入框（中文项目还要让开输入法）** —— `1–9` 选选项、`J` 开手记是从封面就在听的全局键。少了"正在输入就不抢"的判断，后果很具体（实测）：在**设置里改 API 地址**，敲到 `j` 会弹出「手记」浮层、敲 `1–9` 会顺手点掉底下的选项；**沙盘里写行动**同理。两条判断缺一不可：
...
46. **别让"每个调用点都要记得配一句"的样板存在**（批 6：AI 调用收编） —— 原来 `main.js` 的 `callAI()` 一个人管四件事（发请求、思考提示、记账、重试交互），于是 **50 个调用点各自**写 `showThinking(true)` / `finally { showThinking(false) }`、24 处 `st().bumpAiCount()`、每处 `setStepState('busy')`。现在：`modules/ai` 是唯一入口（`ask()`），进度用事件广播（`ai:start/done/fail`），UI 在 `modules/shell`，每类预算在 `modules/ai/registry.js` 一张表里。三条经验（前两条是我自己踩的）：
   ...
   - 顺带：预算与温度随 payload 下发，服务器只做上下界收口并把 `budgetTokens` 记进日志——这样 `qa:ai` 才能核对"表里写的"和"实际用的"是同一件事。**这条对账当天就抓到两处漏网**：`fireSceneGen` 与选项预告、沙盘的两处仍在直连 `decide()/runSimTurn`，它们的预算不受表管（日志里额度是默认 2000 而表里写 1400）。
   - **后台调用给一个 `quiet` 开关**，别用"把提示关掉"的土办法：quiet 只关进度 UI（不弹「思考中」、失败不弹重试面板），**预算/账目/计数照走**。三处后台调用（营地氛围、选项预告、沙盘收尾）就是这么收编的——它们的 UX 一行没变。
