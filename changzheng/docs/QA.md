...
| 档 | 命令 | 什么时候跑 | 特点 |
|---|---|---|---|
| 开发快检 | `npm run dev:check` | 改完一处就扫一眼；批次中间随手跑 | **10 步 / 约 16 秒 / 0 次真调**。静态三关（总线规矩、单元测试、文档与代码一致）+ 一个浏览器一次走完（内核启动 → 开局到营地且 HUD 与存档一致 → 玩法板挂得上 → 输入框不吃快捷键 → 终局失败也不空屏 → 升华可跳过 → 全程无报错）。`--headed` 开窗口看着跑 |
...
| 命令 | 覆盖 | 通过标准 |
|------|------|----------|
| `npm run dev:check` | **批次中间的快检**：总线静态规矩 + 单元测试 + 文档一致性 + 页面侧（内核 boot/契约违规 0/模块全注册/**模块 init·ready·订阅者抛错 0**（内核会捕获跳过，功能静默少一块）/快照冻结/**全屏可插行**/开局到营地且 HUD 与存档一致/两个玩法挂得上/输入框不吃快捷键/终局失败不空屏/**调用流链路通**/无 pageerror） | 10 步全 ✓，约 16 秒，0 次真调 |
| `npm run test:unit` | 资源钳制、史实解锁、附身线门控、行动点、每日场景、失败判定、AI 响应契约、减员规则、数值护栏、同伴与好感一致、诗形制、玩法守卫 | **65/65** 通过 |
...
| `npm run qa:board` | **玩法板体检**：玩法逐屏摆到板屏上（板屏壳/数值签/契约标记/第一步可点/离开清空）+ **三方对账**（玩法清单 ↔ 体检表 ↔ 描述符里声明的 `actions`）。同事插新玩法时，这一节会直接指出缺哪一步 | 114 项全 ✓（14 支玩法） |
...
### 桌面封装（在 `packaging/` 里跑，不属于上面三档）

| 命令 | 覆盖 | 通过标准 |
|------|------|----------|
| `node packaging/verify-fast.mjs` | ① **内容一致**：成品里的 `public/server/data` 与仓库工程**逐文件 sha256 相等**；② **能起来**：本地服务应答、`hasKey=true`、启动日志落在 `user-data/`、单文件版有版本戳；③ **资源齐**：index / JS / 场景图 / 环境音 / 字体 / `data/*.json` 全 200；④ **前端与调用链**：真浏览器开到营地与行程节点、无 JS 报错，直连 `/api/decide` 真调一次并核对 JSONL 落盘，`/api/logs/export` 能下到原始 JSONL、越权文件名被 400 挡 | 四项全 ✓ |
| `node packaging/verify-fast.mjs --single` | 同上，改成验**单文件版**（不重新展开） | 四项全 ✓ |
| `node packaging/verify-fast.mjs --single --fresh` | 删掉展开结果，走真实的「首次双击」 | 四项全 ✓ |
| `node packaging/verify-packaged.mjs` | 发布级全量验收（慢一些）：点着走过一屏 + 真调；`VERIFY_EXE=...` 可指向别的副本 | 通过 |

> 出成品与验收都在 `packaging/` 里跑（`cd packaging`）：`node build.mjs` → `node build-singlefile.mjs`。
> 命令语义、成品目录与外壳行为的唯一真源是 [`../../packaging/README.md`](../../packaging/README.md)；
> **产物与发布口径**（对外只发单文件 exe、走 GitHub Releases）见 [`DELIVERY.md`](DELIVERY.md) §四。
...
- **封装产物也不入库**：`dist/`（便携目录 / zip / 单文件 exe）与 Electron 运行时和打包缓存都不进仓库；
  对外发布走 GitHub Releases 传单文件 exe（见 [`DELIVERY.md`](DELIVERY.md) §四）  
