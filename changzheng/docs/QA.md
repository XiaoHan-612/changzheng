...
| `npm run qa:audio` | **音频逐个体检**（55 个）：容器 vs 扩展名、HTTP 200、MIME、浏览器逐条解码、"能否被代码路径引用到"，外加**框架一致性**（门面唯一、无旧 API 残留、无绕开框架的 `new Audio`/音量写法、**切场景只走声明表**）、**场景表对账**（场景表 kind ↔ 文件映射 ↔ 磁盘文件，缺 BGM/正在兜底都会列出来）与**默认音色一致性**（前端 `DEFAULT_VOICE` ↔ 服务端 `/api/tts` 默认，分岔则 TTS 缓存永远命中不了） | AUDIO PASS，报告落 `docs/AUDIO-REPORT.md` |
