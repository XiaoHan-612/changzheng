...
| `npm run qa:audio` | **音频逐个体检**（55 个）：容器 vs 扩展名、HTTP 200、MIME、浏览器逐条解码、"能否被代码路径引用到"，外加**框架一致性**（门面唯一、无旧 API 残留、无绕开框架的 `new Audio`/音量写法） | AUDIO PASS，报告落 `docs/AUDIO-REPORT.md` |
