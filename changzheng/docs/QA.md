...
| 命令 | 覆盖 | 通过标准 |
|------|------|----------|
| `npm run dev:check` | **批次中间的快检**：总线静态规矩 + 单元测试 + 文档一致性 + 页面侧（内核 boot/契约违规 0/模块全注册/快照冻结/**全屏可插行**/开局到营地且 HUD 与存档一致/两个玩法挂得上/输入框不吃快捷键/终局失败不空屏/**调用流链路通**/无 pageerror） | 9 步全 ✓，约 7 秒，0 次真调 |
