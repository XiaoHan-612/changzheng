...
| `tpl-board` 玩法板 | **全部玩法都在上面**（现共 14 支；清单以 `modules/games/manifest.js` 为准）。壳由玩法宿主 `modules/games/index.js` 起头，玩法状态用区块 `.blk-stat` 写进板头 | 下表是首批 8 支的纸面占比实测：14.9% 弯针 · 29.7% 钓鱼 · 28.7% 夜校 · 21.8% 分糖 · 28.7% 夜岗 · 27.7% 五子棋 · 26.8% 泸定桥 · 15.9% 陡坡 |
...
|---|---|---|
| 墨字对纸底 | ≥7:1 | 8.41:1 |
| **次要文字**：纸面那一档 | ≥4.5:1 | `--ink-note` 5.31:1（暗底那一档 `--muted` 对暗底 5.31:1） |
| 纸面对插画暗部 | ≤9:1 | 7.75:1 |

**次要文字按材料换墨**（2026-09-15）：`--muted` 是为暗底定的，落在纸面上只有 1.86:1（实测，对 `--paper-veil`）——
研学报告里「可以带孩子重点看腊子口…」那种说明句会发虚。修法不是逐处改颜色，而是**材料决定自己内部的次要色**：
`framework.css` 里纸面材料把 `--muted` 重定义为 `--ink-note`（约 5.3:1），墨纱（`.ink-surface`）重定义为 `--paper-dim`。
于是调用点照旧写 `.muted`、不必知道自己在什么表面上，将来新写的也自动是对的，嵌套时由最内层材料决定——
与 `.blk-stat`「落在纸面自动换浅纱」是同一套思路。`qa:tone` 会算这几档的比值，并断言那条覆盖**真的写在 CSS 里**
（删掉它，纸面上的 `.muted` 会静默退回发虚）。
...
| 屏容器 `#screen-*` | 每屏只为自己负责 | **屏自己清理自己的 DOM**：`showScreen` 广播 `screen:hide`，各屏宿主用 `kernel.api('screens').own(屏id, 清理函数)` 登记（**批 3 已落地**）。不许再出现"换屏时去清别人容器"的越界写法（见 [`BUS.md`](BUS.md)） |
