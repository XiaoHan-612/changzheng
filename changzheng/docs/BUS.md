...
| `npm run dev:check` | **加完模块/玩法立刻跑这个**：约 16 秒、0 次真调。它断言"清单里的模块都真的注册上了""契约违规 0""快照已冻结""事件真的被模块收到（`ai:feed` 链路）"，还会把一个玩法摆到板屏上点一下——`wiring.js` 路径写错、事件没登记、描述符导出成坏形状，都在这里当场红 |
