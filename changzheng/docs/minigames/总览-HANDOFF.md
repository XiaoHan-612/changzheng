...
### 引擎与服务端
- `/api/decide`：**19 类** callType（scene_gen / choice_hint / npc_chat / share_judge / minigame_review /
  ...
  school_lesson / school_quiz（夜校）/ candy_scene（分糖））
  —— 权威清单以 `server/schema.js` 的 `REQUIRED` 为准，`tests/unit/schema.test.js` 锁成显式 19 项
...
### 玩法重做批次（14 支"真游戏"版，**未接线**）
- 逐支把主线的 8 个玩法各重做一遍（**决策 + 失败条件 + 真画面**），单独成文件
  `public/js/minigames-<name>.js`（自带小工具，不 import `minigames.js`），一律用**新 id**。
- **铁律**：用户点头前**不改** `acts.json` / `main.js` / `minigames.js` / 注册表。
  逐支交接包在 `docs/HANDOFF-<NAME>.md`；**总览见 [`MINIGAMES-GUIDE.md`](MINIGAMES-GUIDE.md)**。
- **一页全玩**：`/dev/playground.html`（27 支，按幕分组，每张卡写着「决策 / 失败」，深链 `?id=snow-grab`）；
  单支细调看 `/dev/minigame-lab.html`（实时契约面板 + 局内观测量）。
- 本批（2026-09-15/16）完成 **6 支**：`mud-gomoku` · `snow-grab` · `luding-chain` ·
  `pontoon-night` · `stretcher-run` · `rally-river`（收拢 · 湘江东岸，2026-09-16 **替改** `stretcher-run`）。
  逐支有 `npm run qa:<name>` 自动化验收（全绿）；收拢另有 `npm run sim:rally` 难度模拟（不烧模型）。
- **画面路线**：`snow-grab` / `luding-chain` / `rally-river` 3 支已改成**画作当场景**
  （直接用 act 的过场油画 `assets/scenes/*.jpg` 当底图，**一个像素都不重画**）；
  其余仍是自绘 SVG。新增玩法**先查 `ASSETS.md` 有没有现成的画**。
...
`layout-audit --width 820` 与 1280 均零布局缺陷 · 重做玩法 6 支 QA 全绿（见 `MINIGAMES-GUIDE.md` §五）
...
public/js/
  ...
  minigames-*.js 14 支重做玩法（单独开发、未接线；逐支交接包 HANDOFF-<NAME>.md）
...
public/dev/     playground.html（玩法平台，27 支一页全览）· minigame-lab.html（单支调试台）
...
docs/           HANDOFF-*.md 分工交接 · MINIGAMES-GUIDE.md 小游戏总览 · QA.md 命令表
```
