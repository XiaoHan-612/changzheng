...
- 放 `public/audio/poem/`（**该目录不入库**：`.gitignore` 挡着，别人录的/网上找的音频不进公开历史；
  本机与演示机照常能播，怎么放见该目录的 `README.md`）；
- 在 `poem.json` 的 `audio.full` 填文件名，并按录音给每句标 `startMs` / `endMs`；
- 逐字就会跟着录音的已播毫秒走（`voice:progress` 那一路），路线 ① 自动让位。

**格式**：`.ogg`（首选）或 `.wav`。不放 mp3——`qa:audio` 只认这两种容器（会逐个解码核对）。
