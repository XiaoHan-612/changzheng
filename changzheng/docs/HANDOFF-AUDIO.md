...
- 文件：`public/audio/poem/qilv-changzheng.mp3`（《七律·长征(朗诵版)》，**59.3 秒**，44.1kHz 立体声、949,376 字节）；
  **该文件随仓库一起走**（2026-09-16 起从 `.gitignore` 里放出来，算竞赛交付的演示素材）——
  clone 下来就在，不用另外拷；它是什么、怎么换、换完要重算什么，见
  [`public/audio/poem/README.md`](../public/audio/poem/README.md)；
  **对外发布/商用要换掉**：这是外部来源的演示素材、不是本工程原创，换成自有朗读或路线 ① 的逐句 TTS 即可，
  只换文件 + 重新量时间轴，**代码一行不用改**；
...
- 逐字**按挂钟推**（不逐帧读音频位置）：口径是「估个量、文字比音频略早 `POEM_LEAD_MS`（500ms）」——
  **定下的口径是不必精确对齐，宁可字先出、别让人等**（开发期别为逐帧对齐反复返工）；音频只是并行的背景轨，
  响没响都不影响字幕节奏。
