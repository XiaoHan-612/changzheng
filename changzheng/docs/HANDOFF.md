...
### 视觉（纸墨设计系统，逐批打磨 25 页）
- 已完成：框架（模板 + 区块 + 动效 + 守卫）与批 1–3（标题/怎么玩/设置/过场；营地/手记/史实/岔路/回响；舞台对话/抉择/裁决/篝火菜单）
- 待做：批 4–6（小游戏 → 沙盘 → 答题/夜间/终局/记录）。**批四开工前先读 [`DESIGN-SYSTEM.md`](DESIGN-SYSTEM.md) §五 的「批四清单」**——那里列了四个玩法屏的代码位置、四条"类名在样式没了"的必修项、以及"要不要给玩法一个真正的板屏"这个待定决策
