...
| `qa-board.mjs` | `npm run qa:board` — 玩法板体检：**14 支**玩法逐屏摆到板屏上，验板屏壳（题名/数值签）、玩法关键元素、契约标记与"第一步能不能点"，离开板屏是否清干净；含"清单 ↔ 体检表 ↔ 动作声明"三方对账 |
...
| `portrait-sheet.mjs` | 把 14 张立绘拼成一张联系表，核对裁切与用色（改立绘后看这一张） |
| `shot-all.mjs` | 逐屏截图 → `logs/shots/*.png`（**不进仓库**，见根 `.gitignore`）；`node tests/manual/shot-all.mjs` |
| `check-open.mjs` | **打不开排查**：机器上没有 Playwright 时也能跑的静态 / HTTP 体检（文件在不在、静态资源 200 吗） |
| `check-select.mjs` | 「择点穿行」这一屏的点击排查（自选幕次与任务点进得去吗） |
| `qa-css.py` | 历史补丁脚本（用于批量改写上面脚本的路径），已用完，保留备查 |

截图输出：`qa:screens` / 各支体检 → `tests/e2e/artifacts/`（不进仓库）；`shot-all.mjs` → `logs/shots/`（同样不进仓库）。
