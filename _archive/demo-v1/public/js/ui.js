...
export function setAiMode(cfg) {
  ...
  el.textContent = cfg.mockMode ? 'MOCK 演示' : cfg.model || 'GLM-5.1';
...
export function setStageBanner(text, img) {
  ...
  if (img) $('stage-bg').style.backgroundImage = `url('${img}')`;
}
...
export async function say(speaker, text) {
...
export function renderLogs(logs) {
  ...
  $('logs-meta').textContent = `共 ${logs.length} 条调用 · source 可辨 GLM-5.1 / MOCK_AI / FALLBACK`;
  list.innerHTML = logs
    .slice()
    .reverse()
    .map((l) => {
